...
void Camera::update(float delta_time) {
    ...
    update_first_person_controller(delta_time);
}
...
void Camera::update_first_person_controller(float delta_time) {
    InputManager *input = InputManager::get_instance();


    // Si se pulsa el botón derecho del ratón, oculta el cursor
    if (input->is_mouse_button_pressed(MouseButton::RIGHT)) {
        input->hide_and_stop_cursor(true);
    }

    // Si el botón derecho del ratón está pulsado, rota la cámara en función del movimiento del ratón
    if (input->is_mouse_button_down(MouseButton::RIGHT)) {
        vec2 delta = input->get_mouse_delta();
        if (delta != vec2{0}) {
            float mouse_sensitivity = .0007f;
            // Rotación respondiendo al movimiento en el eje x del ratón
            auto new_look_at = rodrigues_rotation(look_at - look_from,
                                                  vec3{0, 1, 0},
                                                  delta.x * mouse_sensitivity);
            // Rotación respondiendo al movimiento en el eje y del ratón
            new_look_at = rodrigues_rotation(new_look_at,
                                             glm::normalize(glm::cross(look_at - look_from, vec3{0, 1, 0})),
                                             -delta.y * mouse_sensitivity);
            look_at = look_from + new_look_at;
        }
        float keyboard_speed = 5.f;
        // Si se pulsa la tecla W, A, S, o D, mueve la cámara en el plano horizontal
        if (input->is_key_down(Key::W) || input->is_key_down(Key::S)) {
            auto delta_vector = glm::normalize(vec3{look_at.x - look_from.x, 0, look_at.z - look_from.z}) * delta_time *
                                keyboard_speed;
            if (input->is_key_down(Key::W)) {
                look_from += delta_vector;
                look_at += delta_vector;
            }
        ...
        if (input->is_key_down(Key::A) || input->is_key_down(Key::D)) {
            auto delta_vector =
                    glm::normalize(glm::cross(look_at - look_from, vec3{0, 1, 0})) * delta_time * keyboard_speed;
        ...
        // Si se pulsa la tecla Espacio o Control izquierdo, mueve la cámara en el eje vertical
        if (input->is_key_down(Key::SPACE) || input->is_key_down(Key::LEFT_CONTROL)) {
            auto delta_vector = vec3{0, 1, 0} * delta_time * keyboard_speed;
            if (input->is_key_down(Key::SPACE)) {
                look_from += delta_vector;
                look_at += delta_vector;
            }
            if (input->is_key_down(Key::LEFT_CONTROL)) {
                look_from -= delta_vector;
                look_at -= delta_vector;
            }
        }
    }
    // Si se suelta el botón derecho del ratón, vuelve a mostrar el cursor
