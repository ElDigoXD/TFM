...
public:
    ...
    /// Controla la rotación de la cámara en primera persona, utilizando el input del usuario.
    /// Permite mover la cámara con W, A, S, D, Space y Ctrl, y rotar la cámara con el ratón.
    /// @param delta_time Tiempo transcurrido desde el último frame, en segundos.
    void update_first_person_controller(float delta_time);
    ...
    /// @note En el caso de WebGPU, el volumen de clip es x: (-1, 1), y: (-1, 1), z: (0, 1).
    /// @note Como optimización, la matriz de proyección se puede pre-calcular y solamente actualizarla cuando se
