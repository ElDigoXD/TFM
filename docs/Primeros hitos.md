> Basado en: https://developer.chrome.com/docs/web-platform/webgpu/build-app
...
Añadir Dawn a `CMakeLists.txt` y crear un programa minimo que muestre una ventana en main.cpp (`main_hello_window.cpp`).

## Paso 5 (Hito 2 - Mostrar un triángulo)

Añadir el código para inicializar WebGPU y mostrar un triángulo (`main_first_triangle.cpp`).

> En el caso de que salte un error al ejecutar el ejecutable de que falta `d3dcompiler_47.dll` o `vulkan-1.dll`,
...
Añadir Emscripten a `CMakeLists.txt` y modificar el código para que se pueda compilar a web (`main_web_triangle.cpp`).

> Hay que cambiar el toolchain de CMake para que utilice el de Emscripten
> 
