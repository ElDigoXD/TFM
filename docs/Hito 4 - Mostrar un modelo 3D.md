...
En `main.cpp` crear un `GameObject`, asignarle el buffer de un objeto 3D cargado mediante `ResourceManager` y, en el bucle del programa, renderizar el objeto utilizando el `Renderer`. Para que el objeto se renderice correctamente en 3D, es necesario añadir un buffer de profundidad a la pipeline.
