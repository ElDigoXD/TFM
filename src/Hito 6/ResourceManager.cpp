...
std::map<const path, wgpu::Texture> ResourceManager::textures{};
...
wgpu::Texture ResourceManager::get_texture(const path &path, const wgpu::Device &device) {
    // Si el modelo existe en el caché, lo devuelve
    ...
    if (!std::filesystem::exists(path)) {
        std::cerr << "ERROR: Could not find texture file " << path << std::endl;
        return {};
    }

    // Carga la imagen utilizando la librería stb_image
    ...
    // Copia los datos de la imagen a la textura
    wgpu::Texture texture = device.CreateTexture(&texture_desc);
    wgpu::TexelCopyTextureInfo tex_info = {
            .texture = texture,
            .mipLevel = 0,
            .origin = {0, 0, 0},
    ...
    wgpu::TexelCopyBufferLayout buffer_layout = {
            .offset = 0,
            .bytesPerRow =  (uint32_t) 4 * width,
            .rowsPerImage = (uint32_t) height,
    };

    device.GetQueue().WriteTexture(&tex_info, data, width * height * 4, &buffer_layout, &texture_desc.size);

    // Libera la memoria de la imagen
    stbi_image_free(data);

    // Guarda la textura en el caché y la devuelve
...
bool ResourceManager::load_obj(const path &path, std::vector<VertexAttributes> &out_vertex_attributes) {
    ...
    bool success = tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err, path.string().c_str(), RESOURCE_DIR);
